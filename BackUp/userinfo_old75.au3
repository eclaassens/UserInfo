...
Global $cancel ; Abort domain search
...
listDomains()
setUser()
GUICtrlSetData($btnCountDeptUsers,"Zoveul: " & countUsers(GUICtrlRead($lblDepartment))
	...
	While 1
		...
		Switch $nMsg
			...
			Case $btnCountDeptUsers
				countUsers(GUICtrlRead($lblDepartment))
			Case -100 To 0
				;
			Case Else
...
Func searchDepartmentUsers()
	...
	While 1
		...
		If ($msg = $btnColleagueSwitch) Or ($msg = $btnColleagueCompare) Then
			$x = GUICtrlRead($userList)
			If $x <> "" Then ; test for empty selection
				$y = GUICtrlRead($x)
				$tmp = StringSplit($y, "|")
				If $msg = $btnColleagueSwitch Then
					GUIDelete($frmColleagues) ; Destroy GUI
					setUser($tmp[2])
					ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : setUser(StringSplit($x, "|")[2]) = ' & setUser(StringSplit($x, "|")[2]) & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
				Else
					GUIDelete($frmColleagues) ; Destroy GUI
					compareUsers($tmp[2])
				EndIf
