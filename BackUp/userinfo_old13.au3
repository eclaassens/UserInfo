...
$valueWidthShort = 30 ; default value short width
$valueWidthDate = 125 ; Width for date fields
...
GUICtrlCreateLabel("/", $cellPadding + $col1ValueLeft + $valueWidthShort + $cellPadding, $top, 5, $labelHeight)
...
$lbl_erzeugt = GUICtrlCreateLabel("", $col2ValueLeft, $top, $valueWidthDate, $valueHeight, $SS_SUNKEN)
GUICtrlSetFont(-1, -1, 800)
	...
	For $oACE In $oACL
		If ($oACE.ObjectType = $USER_CHANGE_PASSWORD) And (($oACE.Trustee = "Everyone") Or ($oACE.Trustee = "Iedereen")) Then
			If ($oACE.AceType = $ADS_ACETYPE_ACCESS_ALLOWED_OBJECT) Then
				...
				GUICtrlSetColor($lblPWChange, 0x00aa00)
			Else
				GUICtrlSetData($lblPWChange, "Locked")
				GUICtrlSetColor($lblPWChange, 0xff0000)
			EndIf
		EndIf
	Next
