...
Func searchDepartmentUsers()
	...
	; Create GUI
	$frmNeighbours = GUICreate("Show department neighbours", 550, 510)
	$userList = GUICtrlCreateListView("Office|Username|UserID|Telephone", 5, 10, 540, 400)
	_GUICtrlListView_SetColumnWidth($userList, 0, 70)
	_GUICtrlListView_SetColumnWidth($userList, 1, 250)
	...
	Do
		...
		If Not StringInStr($y, "ou=Recipients") Then ; pass over all that exists only as a mail receiver (which are namely ou=users, ou=receiver)
			...
			_ArrayAdd($arrayFound, $o_temp_Usr.physicalDeliveryOfficeName & "|" & $o_temp_Usr.sn & "," & $o_temp_Usr.givenName & "|" & $o_temp_Usr.samAccountName & "|" & $o_temp_Usr.telephoneNumber, 0, "|")
			ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : 			_ArrayAdd($arrayFound, $o_temp_Usr.physicalDeliveryOfficeName & "|" & $o_temp_Usr.sn & "," & $o_temp_Usr.givenName & "|" & $o_temp_Usr.samAccountName & "|" & $o_temp_Usr.telephoneNumber, 0, "|") = ' & 			_ArrayAdd($arrayFound, $o_temp_Usr.physicalDeliveryOfficeName & "|" & $o_temp_Usr.sn & "," & $o_temp_Usr.givenName & "|" & $o_temp_Usr.samAccountName & "|" & $o_temp_Usr.telephoneNumber, 0, "|") & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
		EndIf
		$objRecordSet.MoveNext
	Until $objRecordSet.EOF
