...
Opt("GUICloseOnESC", 0)
;Opt("GUICoordMode", 2)
;Opt("GUIResizeMode", 0) ;0=no resizing, <1024 special resizin
...
Func _ADGetGroupMembers($group)
	...
	_ArraySort($arrayMembers) ; sort on Displayname
	$counter = 0
	$result1 = ""
	For $strMember = 0 To UBound($arrayMembers) - 1
		$result1 = $result1 & "<td bgcolor=66ff66>" & $arrayMembers[$counter][0] & "<br>(" & $arrayMembers[$counter][1] & ")</td>"
		$counter = $counter + 1
	...
	For $strMember = 0 To UBound($arrayMembers) - 1
		...
		$result2 = $result2 & "<td bgcolor=66ff66>" & $arrayMembers[$counter][1] & "<br>(" & $arrayMembers[$counter][0] & ")</td>"
		$counter = $counter + 1
	...
	FileWriteLine($data, "<tr><th bgcolor=ff6666>" & "Created" & ":</th><td bgcolor=66ff66 colspan=10>" & $created & "</td></tr>")
	FileWriteLine($data, "<tr><th bgcolor=ff6666>" & "Last changed" & ":</th><td bgcolor=66ff66 colspan=10>" & $changed & "</td></tr>")
	...
	FileWriteLine($data, "<tr><th bgcolor=ff6666>" & "Member count" & ":</th><td bgcolor=66ff66 colspan=10>" & UBound($arrayMembers) - 1 & "</td></tr>")
	...
	Switch $objRecordSet.RecordCount
		...
		Case Else ; User existiert, aber mehrere Treffer (Suchbegriff: Meier ^^)
			...
			Else ; frmUserSelect - Show this form to select the right user
				...
				$liste = GUICtrlCreateListView("Name or UserID to search", 5, 40, 340, 280)
...
Func compareGroup($compareUser = "")
	...
	; investigating the maximum number of entries for all 3 columns
	$max = 0
	$m1 = UBound($missing1)
	$m2 = UBound($missing2)
...
; 	FileWriteLine($data, ".table thead th{ padding:15px; color:#fff; text-shadow:1px 1px 1px #568F23; border:1px solid #93CE37; border-bottom:3px solid #9ED929; background-color:#9DD929; background:-webkit-gradient( linear, left bottom, left top, color-stop(0.02, rgb(123,192,67)), color-stop(0.51, rgb(139,198,66)), color-stop(0.87, rgb(158,217,41)) ); background: -moz-linear-gradient( center bottom, rgb(123,192,67) 2%, rgb(139,198,66) 51%, rgb(158,217,41) 87% ); -webkit-border-top-left-radius:5px; -webkit-border-top-right-radius:5px; -moz-border-radius:5px 5px 0px 0px; border-top-left-radius:5px; border-top-right-radius:5px; }")
; 	FileWriteLine($data, ".table tbody th{ color:#fff; background-color:#9DD929; border:1px solid #93CE37; padding:0px 10px; -moz-border-radius:5px 5px 5px 5px; -webkit-border-radius:5px; -webkit-border-radius:5px; border-radius:5px; }")
; 	FileWriteLine($data, ".table tbody td{ padding:10px; text-align:center; background-color:#DEF3CA; border: 2px solid #E7EFE0; -moz-border-radius:2px; -webkit-border-radius:2px; border-radius:2px; color:#666; }")
; 	FileWriteLine($data, ".table tfoot td{ color: #9CD009; font-size:10px; text-align:center; padding:4px 0px; }")
; 	FileWriteLine($data, "</style>")
; 	FileWriteLine($data, "<html> <body> <table class=""table"">")
; 	; write header
 #CE
	FileWrite($data, '
	<style>
table.table{ font-family: "Trebuchet MS", sans-serif; font-size: 12px; font-weight: bold; line-height: 1.4em; font-style: normal; border-collapse:separate; }
.table thead th{ padding:8px 0px; color:#fff; text-shadow:1px 1px 1px #568F23; border:1px solid #93CE37; border-bottom:3px solid #9ED929; background-color:#9DD929; background:-webkit-gradient( linear, left bottom, left top, color-stop(0.02, rgb(123,192,67)), color-stop(0.51, rgb(139,198,66)), color-stop(0.87, rgb(158,217,41)) ); background: -moz-linear-gradient( center bottom, rgb(123,192,67) 2%, rgb(139,198,66) 51%, rgb(158,217,41) 87% ); -webkit-border-top-left-radius:5px; -webkit-border-top-right-radius:5px; -moz-border-radius:5px 5px 0px 0px; border-top-left-radius:5px; border-top-right-radius:5px; }
.table tbody th{ padding:0px 5px; color:#fff; background-color:#9DD929; border:1px solid #93CE37;  -moz-border-radius:5px 5px 5px 5px; -webkit-border-radius:5px; -webkit-border-radius:5px; border-radius:5px; }
.table tbody td{ padding:0px 5px; text-align:left; background-color:#DEF3CA; border: 2px solid #E7EFE0; -moz-border-radius:2px; -webkit-border-radius:2px; border-radius:2px; color:#666; }
.table tfoot td{ padding:2px 0px; text-align:center; color: #9CD009; font-size:10px;   }
</style>
<html> <body> <table class="table">
	')
	FileWriteLine($data, "<thead> <tr> <th>Groups which only " & $user & " has</th> <th>Common Groups</th> <th>Groups which only " & $compareUser & " has</th> </tr> </thead>")
	; write rows
	For $i = 1 To $max
		$s1 = ""
		$s2 = ""
	...
	Next
	; write footer
	FileWriteLine($data, "<tfoot> <tr> <td>" & $m1 & "</td> <td>" & $m3 & "</td> <td>" & $m2 & "</td> </tr> </tfoot> ")
	; write end tags
	FileWriteLine($data, "</table> </body> </html>")
	FileClose($data)
	ShellExecute(@TempDir & "\userinfo.htm")
...
Func searchDepartment()
	...
	; Create GUI
	$frmNeighbours = GUICreate("Show neighbours", 550, 510)
	...
	$btnNeighbours_compare = GUICtrlCreateButton("Compare groups with this neighbour", 5, 415, 540, 20)
	...
	Do
		...
		If Not StringInStr($y, "ou=Recipients") Then ; pass over all that exists only as a mail receiver (which are namely ou=users, ou=receiver)
	...
	For $i = 0 To UBound($arrayFound) - 1 ; copy all results in ListView
		$x = GUICtrlCreateListViewItem($arrayFound[$i], $liste)
		$wert = StringSplit($arrayFound[$i], "|")
		If $wert[1] = $office Then GUICtrlSetBkColor($x, 0xffff00) ; color the room number of the user
	Next

	GUISetState(@SW_SHOW, $frmNeighbours)

	While 1
		$msg = GUIGetMsg()
		If ($msg = $btnNeighbours_wechseln) Or ($msg = $btnNeighbours_compare) Then
			$x = GUICtrlRead($liste)
			If $x <> "" Then ; nur wenn auch ein User gewählt wurde...
				$y = GUICtrlRead($liste) ; die Control-ID des markierten Listview-Items holen
				$y = GUICtrlRead($y) ; das ListviewItem auslesen
				...
				If $msg = $btnNeighbours_wechseln Then
					GUIDelete($frmNeighbours) ; Destroy GUI
					setUser($tmp[3])
				Else
					GUIDelete($frmNeighbours) ; Destroy GUI
					compareGroup($tmp[3])
