...
#include <File.au3>
#include <Math.au3>; import the max command
#include <Misc.au3> ; import _singleton
...
$objConnection.Open("Active Directory Provider") ; Open AD connection

Global $connEmpirum = ObjCreate("ADODB.Connection") ; empirum connection
Global $connNCDaten = ObjCreate("ADODB.Connection") ; NC data connection
Global $connSBDAT = ObjCreate("ADODB.Connection") ; Sachbearbeiter-Datei Verbindung
Global $rs = ObjCreate("ADODB.Recordset") ; Recordset for search results in the databases
Global $ADSystemInfo = ObjCreate("ADSystemInfo")
Global $anzahl_pcs
Global $anzahl_ncs

#Region  ; User Interface
$appTitel = "Userinfo v2.79"
If @AutoItX64 Then
	$appTitel &= " [64 Bit]"
Else
	$appTitel &= " [32 Bit]"
...
If @Compiled And _Singleton("userinfo", 1) = 0 Then ; If already running just open the running instance
	WinActivate($appTitel)
	Exit
EndIf

$Form1 = GUICreate($appTitel, 980, 800, -1, -1)
GUISetBkColor(0xeeeeee)
GUICtrlSetFont($Form1, 6)
$row1 = 16 ; Start der 1. label
$row2 = 110 ; Start der 1. Werte
$row3 = 410 ; Start der 2. label
$row4 = 480 ; Start der 2. Werte
$width1 = 370 ; width der 1. Werte
$width2 = 180 ; width der 2. Werte
$width3 = 60
$width4 = 220

Global Const $buttonrow = 825
...
$btn_refresh = GUICtrlCreateButton($texts[$language][2], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 255)
$y += 40

$btn_nachbarn = GUICtrlCreateButton($texts[$language][3], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, @SystemDir & "\shell32.dll", 269)
$y += 40

$btnChangeUser = GUICtrlCreateButton($texts[$language][4], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 23)
$y += 60

$bn_overview_copy = GUICtrlCreateButton($texts[$language][5], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 223)
$y += 40

$bn_overview_print = GUICtrlCreateButton($texts[$language][6], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 17)
$y += 80

$bn_vnc_nc = GUICtrlCreateButton($texts[$language][7], $buttonrow, $y, $buttonwidth, 20)
$y += 20

$bn_citrix = GUICtrlCreateButton($texts[$language][8], $buttonrow, $y, $buttonwidth, 20)
$y += 30

$bn_userzaehler = GUICtrlCreateButton($texts[$language][9], $buttonrow, $y, $buttonwidth, 20)
$y += 20
$bn_dezernat = GUICtrlCreateButton($texts[$language][10], $buttonrow, $y, $buttonwidth, 20)
$y += 70

$btnGroupCompare = GUICtrlCreateButton($texts[$language][11], $buttonrow, $y, $buttonwidth, 20)
$y += 20

$bn_gruppen = GUICtrlCreateButton($texts[$language][12], $buttonrow, $y, $buttonwidth, 20)
$y += 20

$bn_gruppenmitglieder = GUICtrlCreateButton($texts[$language][13], $buttonrow, $y, $buttonwidth, 20)
$y += 60

$bn_laufwerk_c = GUICtrlCreateButton($texts[$language][14], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 9)
$y += 40

$bn_eventvwr = GUICtrlCreateButton($texts[$language][15], $buttonrow, $y, $buttonwidth, 40)
GUICtrlSetImage(-1, "shell32.dll", 330)
$y += 40

$bn_remote = GUICtrlCreateButton($texts[$language][16], $buttonrow, $y, $buttonwidth, 20)
$y += 20
$bn_versionsinfo = GUICtrlCreateButton($texts[$language][17], $buttonrow, $y, $buttonwidth, 20)
$y += 20

$bn_software = GUICtrlCreateButton($texts[$language][18], $buttonrow, $y, $buttonwidth, 20)
$y += 65

$bn_ende = GUICtrlCreateButton($texts[$language][19], $buttonrow, $y, $buttonwidth, 40)
...
$y = 10
GUICtrlCreateLabel($texts[$language][20], $row1, $y, 90, 17)
$lbl_name = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)
GUICtrlCreateLabel($texts[$language][21], $row3, $y, $width3, 17)
$lbl_userid = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][22], $row1, $y, 90, 17)
$lbl_anrede = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)
GUICtrlCreateLabel($texts[$language][23], $row3, $y, $width3, 17)
$lbl_fnummer = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][24], $row1, $y, 90, 17)
$lbl_ou = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][25], $row3, $y, $width3, 17)
$lbl_telefon = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][26], $row1, $y, 69, 17)
$lbl_department = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][27], $row3, $y, $width3, 17)
$lbl_domain = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][28], $row1, $y, 28, 17)
$lbl_email = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][29], $row3, $y, 69, 17)
$lbl_ort = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][30], $row1, $y, 90, 17)
$lbl_adresse = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][31], $row3, $y, 69, 17)
$lbl_room = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)
...
$width4 = 330
$y += 90
GUICtrlCreateLabel($texts[$language][33], $row1, $y, 90, 17)
$lbl_aenderung = GUICtrlCreateLabel("", $row2, $y, 130, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][34], $row1 + 220, $y, 55, 17)
$lbl_expiration = GUICtrlCreateLabel("", $row1 + 280, $y, 130, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][35], $row3, $y, $width3, 17)
$lbl_pwreq = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20

GUICtrlCreateLabel($texts[$language][36], $row3, $y, $width3, 17)
$lbl_change = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][37], $row1, $y, 85, 17)
$lbl_Locked = GUICtrlCreateLabel("", $row2, $y, 30, 17)
GUICtrlSetFont(-1, -1, 800)
GUICtrlCreateLabel("/", $row2 + 30, $y, 5, 17)
$lbl_deakt = GUICtrlCreateLabel("-", $row2 + 40, $y, 50, 17)
GUICtrlSetFont(-1, -1, 800)

$bn_unlock = GUICtrlCreateButton($texts[$language][38], 200, $y - 4, 90, 20)

GUICtrlCreateLabel($texts[$language][39], $row3, $y, $width3, 17)
$lbl_erzeugt = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][40], $row1, $y, 85, 17)
$lbl_errorcount = GUICtrlCreateLabel("", $row2, $y, 85, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][41], $row3, $y, $width3, 17)
$lbl_login = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][42], $row1, $y, 89, 17)
$lbl_homeverz = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][43], $row3, $y, $width3, 17)
$lbl_homelauf = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][44], $row1, $y, 55, 17)
$lbl_script = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][45], $row3, $y, $width3, 17)
$lbl_profile = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][46], $row1, $y, 85, 17)
$lbl_geboren = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][47], $row3, $y, 55, 17)
$lbl_TSProfile = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][48], $row1, $y, 85, 17)
$lbl_postkorb = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][49], $row3, $y, 55, 17)
$lbl_AmtsBez = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][50], $row1, $y, 85, 17)
$lbl_pwchange = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)

GUICtrlCreateLabel($texts[$language][51], $row3, $y, 55, 17)
$lbl_geschlecht = GUICtrlCreateLabel("", $row4, $y, $width4, 17)
GUICtrlSetFont(-1, -1, 800)

$y += 20
GUICtrlCreateLabel($texts[$language][52], $row1, $y, 85, 17)
$lbl_SAP_PNR = GUICtrlCreateLabel("", $row2, $y, $width1, 17)
GUICtrlSetFont(-1, -1, 800)
...
$Form2 = GUICreate($appTitel, 370, 140, -1, -1)
...
Func domainList() ; get all known Domains
	$oDS = ObjGet("WinNT:")
	$y = ""
	For $x In $oDS
		$y = $y & "|" & $x.Name
	Next
	GUICtrlSetData($domain, $y, "NL")
EndFunc   ;==>domainList

Func setUser($userid_tmp = "")
	ControlFocus($appTitel, "", $btnChangeUser)
	...
	If $userid_tmp <> "" Then
		...
		$loopflag = search_domain()
	EndIf

	If $loopflag = 0 Then
		ControlClick($appTitel, "", $edit)
		Send("+{home}")
		$loopflag = 1
		While $loopflag
			$msg = GUIGetMsg()
	...
	While 1
		...
		Switch $nMsg
			...
			Case $bn_userzaehler
				userCount()
			Case $bn_laufwerk_c
				$x = pc_bestimmen() ; besorge Dir den Namen des gewünschten PCs, ggf. beim User nachfragen wenn mehrere Vorhanden
				If $x <> "0" Then besuchen("\\" & $x & "\c$") ; wenn PC erfolgreich gewählt, öffne das C-Laufwerk
			Case $bn_eventvwr
				$x = pc_bestimmen() ; besorge Dir den Namen des gewünschten PCs, ggf. beim User nachfragen wenn mehrere Vorhanden
				If $x <> "0" Then Run("eventvwr \\" & $x, @SystemDir) ; wenn PC erfolgreich gewählt, öffne die Ereignisanzeige
			Case $bn_versionsinfo
				$x = pc_bestimmen() ; besorge Dir den Namen des gewünschten PCs, ggf. beim User nachfragen wenn mehrere Vorhanden
				If $x <> "0" Then versionsinfo($x) ; wenn PC erfolgreich gewählt, öffne das C-Laufwerk
			Case $bn_software
				$x = pc_bestimmen() ; besorge Dir den Namen des gewünschten PCs, ggf. beim User nachfragen wenn mehrere Vorhanden
				If $x <> "0" Then softwareliste($x) ; wenn PC erfolgreich gewählt, öffne das C-Laufwerk
			Case $bn_remote
				$x = pc_bestimmen() ; besorge Dir den Namen des gewünschten PCs, ggf. beim User nachfragen wenn mehrere Vorhanden
				If $x <> "0" Then remote_desktop($x)
			Case -100 To 0
				;
			Case Else
				...
				If $tmp <> "" Then ; nur, wenn auch ein Text erfasst wurde
					...
					$x = ControlGetFocus($appTitel) ; finde heraus, worauf geklickt wurde
...
Func search_domain($user = "", $domain = "")
	If $user = "" Then $user = GUICtrlRead($edit)
	If $domain = "" Then $domain = GUICtrlRead($domain)
	...
	If $user <> "" Then
		...
		Switch _ADObjectExists($user)
			Case 1 ; User found
				...
				Return 1
			Case -1 ; Keyword yielded several hits, so the user ID was entered in the form
				ControlClick($appTitel, "", $btn_ok)
				Return 0 ; show the little window, but now with the unique user ID
...
Func _ADGetUserData($user)
	...
	GUICtrlSetData($lbl_department, StringReplace($oUsr.department, "&", "&&"))
	GUICtrlSetData($lbl_profile, $oUsr.profilePath)
	...
	GUICtrlSetData($lbl_room, $oUsr.physicalDeliveryOfficeName)
	GUICtrlSetData($lbl_adresse, $oUsr.streetAddress)

	If (@OSArch = "X86") Or (@OSArch = "X64" And @AutoItX64 = 1) Then
		GUICtrlSetData($lbl_TSProfile, $oUsr.TerminalServicesProfilePath)
	Else
		GUICtrlSetData($lbl_TSProfile, $texts[$language][73])
	EndIf
EndFunc   ;==>_ADGetUserData

Func DB_Abfrage($user) ;DELETE
	Return
	...
	$find = "SELECT EBK, GebDatum,Geblist, PWChange FROM xxxxxxxxxxx WHERE UserID LIKE '" & $user & "'"
	$rs.Open($find, $connSBDAT)
	...
	$find = "SELECT Postkorb, AmtsBez, Geschlecht, SAP_PNR FROM T_DB2 WHERE UserID LIKE '" & $user & "'"
	$rs.Open($find, $connSBDAT)
...
Func _ADGetUserGroups(ByRef $usergroups, $user = @UserName)
	...
	Next
	GUICtrlSetData($bn_gruppen, $texts[$language][12] & " (" & $count & ")")
...
Func _ADGetAccount($user)
	...
	If $oUsr.IsAccountLocked Then
		GUICtrlSetData($lbl_Locked, $texts[$language][74])
		GUICtrlSetColor($lbl_Locked, 0xff0000)
		GUICtrlSetState($bn_unlock, $GUI_ENABLE)
	Else
		GUICtrlSetData($lbl_Locked, $texts[$language][75])
		GUICtrlSetColor($lbl_Locked, 0x00aa00)
		GUICtrlSetState($bn_unlock, $GUI_DISABLE)
	EndIf
...
Func unlock($user)
	If $oUsr.IsAccountLocked Then
		...
		If Not $oUsr.IsAccountLocked Then
			GUICtrlSetData($lbl_Locked, $texts[$language][75])
			GUICtrlSetColor($lbl_Locked, 0x00aa00);Green
			GUICtrlSetState($bn_unlock, $GUI_DISABLE)
		Else
			GUICtrlSetData($lbl_Locked, $texts[$language][74])
			GUICtrlSetColor($lbl_Locked, 0xff0000) ; Red
			GUICtrlSetState($bn_unlock, $GUI_ENABLE)
		EndIf
	EndIf
EndFunc   ;==>unlock
...
Func CompareGroup($compareUser = "")
	...
	For $i = 0 To _GUICtrlListView_GetItemCount($Gruppenliste) - 1 ; durchlaufe alle Gruppen des geöffneten Users
		$flag = 0 ; per default hat der User die Gruppe, der Vergleichsuser nicht
		$name1 = _GUICtrlListView_GetItem($Gruppenliste, $i, 0)
		$name1 = $name1[3]
		For $name2 In $vergleichsgruppen ; durchlaufe alle Gruppen des Vergleichs-Users
			If $name1 = $name2 Then ; wenn der Name der aktuellen Gruppe vom User auch beim Vergleichsuser vorhanden ist
				...
				$gemeinsame = $gemeinsame & $name1 & @CRLF ; und füge den Gruppennamen zur Liste der gemeinsamen Gruppen hinzu
		...
		If $flag = 0 Then $fehlende1 = $fehlende1 & $name1 & @CRLF
	Next

	For $name2 In $vergleichsgruppen ; durchlaufe alle Gruppen des Vergleichsusers
		$flag = 0
		For $i = 0 To _GUICtrlListView_GetItemCount($Gruppenliste) - 1 ; durchlaufe die Gruppen des geöffneten Users
			$name1 = _GUICtrlListView_GetItem($Gruppenliste, $i, 0) ; hole den ausgeschriebenen (!) Gruppennamen aus der 1. Spalte
			$name1 = $name1[3]
			If $name2 = $name1 Then
				$flag = 1
				ExitLoop
			EndIf
		Next
		If $flag = 0 Then $fehlende2 = $fehlende2 & $name2 & @CRLF
	Next

	$fehlende1 = StringSplit($fehlende1, @CRLF)
	$fehlende2 = StringSplit($fehlende2, @CRLF)
	$gemeinsame = StringSplit($gemeinsame, @CRLF)

	; kürze bei den beiden Listen der Gruppen die nur 1 User hat, die ersten 2 Zeilen (Anzahl der Elemente und ein Leerfeld) weg
	;	_ArrayDelete($fehlende1, 0)
	_ArrayDelete($fehlende2, 0)
	;	_ArrayDelete($fehlende1, 0)
	_ArrayDelete($fehlende2, 0)
	...
	For $i = 1 To $max
		...
		$s2 = ""
		$s3 = ""
		If $i < $m1 Then $s1 = $fehlende1[$i]
		If $i < $m2 Then $s2 = $fehlende2[$i]
		If $i < $m3 Then $s3 = $gemeinsame[$i]
...
Func print($modus)
	...
	For $row = 0 To _GUICtrlListView_GetItemCount($computer)
		...
		$tmp1 = _GUICtrlListView_GetItem($computer, $row, 6) ; Get Mac-Adress, Länge ist konstant
		$tmp2 = $tmp1[3]
		$ncs = $ncs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $row, 7) ; Get Onlinestatus
		$tmp2 = $tmp1[3]
		$ncs = $ncs & " " & $tmp2
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][21]) & ": " & GUICtrlRead($lbl_userid) & @CRLF
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][27]) & ": " & GUICtrlRead($lbl_domain) & @CRLF
	...
	$res = $res & @CRLF
	$res = $res & StringFormat("[%-22s]", $texts[$language][26]) & ": " & GUICtrlRead($lbl_department) & @CRLF
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][31]) & ": " & GUICtrlRead($lbl_room) & @CRLF
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][98]) & ": " & GUICtrlRead($lbl_Locked) & @CRLF
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][45]) & ": " & GUICtrlRead($lbl_profile) & @CRLF
	$res = $res & StringFormat("[%-22s]", $texts[$language][47]) & ": " & GUICtrlRead($lbl_TSProfile) & @CRLF
	$res = $res & @CRLF
	...
	$res = $res & StringFormat("[%-22s]", $texts[$language][103]) & ": " & UBound($groups) - 1
	$res = $res & @CRLF & "==================================================" & @CRLF
...
Func Empirum() ; DELETE
	...
	$find = "SELECT distinct Computername, ipaddress, macaddress FROM dmisystem AS dmi INNER JOIN InvComputer AS cl ON dmi.client_id=cl.client_id WHERE oslogin LIKE '" & $user & "' ORDER BY Computername"
	$rs.Open($find, $connEmpirum)
	While Not $rs.EOF
		$anzahl_pcs += 1
	...
	$find = "SELECT distinct Computername, ipaddress, macaddress FROM dmisystem AS dmi INNER JOIN InvComputer AS cl ON dmi.client_id=cl.client_id WHERE Computername LIKE '" & $temp & "' ORDER BY Computername"
...
Func NC_Daten() ; DELETE
	...
	$find = "SELECT Name, ip_address, mac_address FROM device WHERE info3 LIKE '" & $tmp & "' ORDER BY Name"
	$rs.Open($find, $connNCDaten)
	$Text = ""
	$row = 0
	...
	$find = "SELECT Name, ip_address, mac_address FROM device WHERE Name LIKE '" & $tmp & "' ORDER BY Name"
	$rs.Open($find, $connNCDaten)
	$Text = ""
...
Func suche_nachbarn() ;DELETE
	$room = $oUsr.physicalDeliveryOfficeName
	$room = StringLeft($room, StringLen($room) - 3) & "*"
	...
	Dim $treffer_array[1]
	$z = ""
	$strQuery = "<LDAP://" & $strHostServer & "/" & $strDNSDomain & ">;(&(objectCategory=person)(objectclass=user)(physicalDeliveryOfficeName=" & $room & "));ADsPath;subtree"
	$objRecordSet = $objConnection.Execute($strQuery) ; Retrieve the FQDN if it exists
	Do
		$y = $objRecordSet.Fields(0).Value ; FQDN-Name des Users
	...
	For $i = 0 To UBound($treffer_array) - 1 ; und alle Ergebnisse in Listview kopieren
		...
		If $wert[1] = $room Then GUICtrlSetBkColor($x, 0xffff00) ; die Raumnummer des Users einfärben
	Next
...
Func pc_bestimmen() ;DELETE
	$tmp = ""
	If $anzahl_pcs = 1 Then
		$tmp = _GUICtrlListView_GetItem($computer, 0, 0)
		$tmp = $tmp[3]
	EndIf
	...
	If $tmp = "" Then
		...
		If $y <> 0 Then
			$y = GUICtrlRead($y) ; das ListviewItem auslesen
			$tmp = StringSplit($y, "|") ; aufteilen und...
			$tmp = $tmp[1];  nur den Namen des PCs behalten
		EndIf
	EndIf
	...
	If $tmp = "" Then ; es gibt mehr als 1 potentielles Gerät zur Auswahl und es war keines angeklickt -> frage nach
		...
		For $i = 0 To _GUICtrlListView_GetItemCount($computer)
			$tmp2 = _GUICtrlListView_GetItem($computer, $i, 0) ; Name des PCs
			$tmp3 = _GUICtrlListView_GetItem($computer, $i, 1) ; IP
			$temp = $tmp2[3] & "|" & $tmp3[3]
			GUICtrlCreateListViewItem($temp, $liste)
		Next
		...
		WEnd
	EndIf
	Return $tmp ; übergebe den Namen des PCs zurück
EndFunc   ;==>pc_bestimmen

Func versionsinfo($pc_name) ; DELETE
...
Func softwareliste($pc_name) ;DELETE
	If Not Ping($pc_name) Then
		MsgBox(16, $texts[$language][118], $texts[$language][119])
		Return
	EndIf
...
Func remote_desktop($pc_name) ;DELETE
...
Func findDepartment()
	...
	While 1
		...
		EndIf
	WEnd
EndFunc   ;==>findDepartment

Func suche_postkorb($searchTerm) ;DELETE!
	Return
	$connSBDAT.Open("Driver={SQL Server};Server=sqlserv.xxxxxxx.de;Database=X500;") ; Connect to data base
	If @error Then
		SplashTextOn("", $texts[$language][125], 250, 100)
		Sleep(2000)
		SplashOff()
		Return
	EndIf
	$find = "SELECT UserID FROM T_DB2 WHERE Postkorb LIKE '" & $searchTerm & "'"
	$rs.Open($find, $connSBDAT)
