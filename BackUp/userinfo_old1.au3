...
$bn_refresh = GUICtrlCreateButton($texts[$language][2], $buttonrow, $y, $buttonwidth, 40)
...
$btn_userwechsel = GUICtrlCreateButton($texts[$language][4], $buttonrow, $y, $buttonwidth, 40)
...
$bn_gruppenvergleich = GUICtrlCreateButton($texts[$language][11], $buttonrow, $y, $buttonwidth, 20)
$y += 20
...
Func domainList() ; get all known Domains - and, in our case, select ours
	$oDS = ObjGet("WinNT:")
	$y = ""
	For $x In $oDS
		$y = $y & "|" & $x.Name
...
Func CompareGroup($vergleichsuser = "")
	Local $strQuery2, $objRecordSet2, $ldap_entry2
	Dim $vergleichsgruppen

	If $vergleichsuser = "" Then
		$vergleichsuser = InputBox($texts[$language][92], $texts[$language][93], "", "", 300, 120)
		If @error Then Return
	EndIf

	$strQuery2 = "<LDAP://" & $strHostServer & "/" & $strDNSDomain & ">;(sAMAccountName=" & $vergleichsuser & ");ADsPath;subtree"
	...
	$datei = FileOpen(@TempDir & "\userinfo.htm", 2)
	FileWriteLine($datei, "<table>")
	FileWriteLine($datei, "<tr><th bgcolor=ff6666>" & $texts[$language][94] & $user & $texts[$language][95] & "</th><th bgcolor=66ff66>" & $texts[$language][96] & "</th><th bgcolor=ff6666>" & $texts[$language][94] & $vergleichsuser & $texts[$language][95] & "</th></tr>")
	For $i = 1 To $max
		$s1 = ""
		$s2 = ""
		$s3 = ""
...
EndFunc   ;==>Groupcompare

Func drucken($modus)

	$pcs = ""
	$ncs = ""
	For $zeile = 0 To _GUICtrlListView_GetItemCount($computer)
		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 0) ; PC-Namen auslesen und auf 35 Zeichen erweitern
		$tmp2 = $tmp1[3]
		$tmp2 = $tmp2 & _StringRepeat(" ", 25 - StringLen($tmp2))
		$pcs = $pcs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 1) ; IP auslesen und auf 16 Zeichen erweitern
		$tmp2 = $tmp1[3]
		$tmp2 = $tmp2 & _StringRepeat(" ", 16 - StringLen($tmp2))
		$pcs = $pcs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 2) ; Mac-Adresse auslesen, Länge ist konstant
		$tmp2 = $tmp1[3]
		$pcs = $pcs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 3) ; Onlinestatus auslesen
		$tmp2 = $tmp1[3]
		$pcs = $pcs & " " & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 4) ; NC-Namen auslesen und auf 35 Zeichen erweitern
		$tmp2 = $tmp1[3]
		$tmp2 = $tmp2 & _StringRepeat(" ", 25 - StringLen($tmp2))
		$ncs = $ncs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 5) ; IP auslesen und auf 16 Zeichen erweitern
		$tmp2 = $tmp1[3]
		$tmp2 = $tmp2 & _StringRepeat(" ", 16 - StringLen($tmp2))
		$ncs = $ncs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 6) ; Get Mac-Adress, Länge ist konstant
		$tmp2 = $tmp1[3]
		$ncs = $ncs & $tmp2

		$tmp1 = _GUICtrlListView_GetItem($computer, $zeile, 7) ; Get Onlinestatus
		$tmp2 = $tmp1[3]
		$ncs = $ncs & " " & $tmp2
...
Func Empirum() ; Auslesen der PCs über die Empirum Datenbank
	Return
	...
	While Not $rs.EOF
		...
		_GUICtrlListView_AddSubItem($computer, $zeile, $x, 0)
		_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("ipaddress").Value, 3), 1)
		_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("macaddress").Value, 3), 2)
		If Not Ping($x, 50) Then
			_GUICtrlListView_AddSubItem($computer, $zeile, "off", 3)
		Else
			_GUICtrlListView_AddSubItem($computer, $zeile, "on", 3)
		EndIf
		$zeile += 1
		If $zeile > 9 Then ExitLoop
		$rs.MoveNext
	WEnd
	$rs.Close
	...
	While Not $rs.EOF
		...
		If _GUICtrlListView_FindItem($computer, -1, $tInfo, $tmp) = -1 Then ; nur wenn pc noch nicht drin steht
			...
			$x = StringStripWS($rs.Fields("Computername").Value, 3)
			GUICtrlCreateListViewItem("", $computer)
			_GUICtrlListView_AddSubItem($computer, $zeile, $tmp, 0)
			_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("ipaddress").Value, 3), 1)
			_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("macaddress").Value, 3), 2)
			If Not Ping($x, 50) Then
				_GUICtrlListView_AddSubItem($computer, $zeile, "off", 3)
...
Func NC_Daten() ; Auslesen der NCs über die NC-Datenbank
	Return
	If @AutoItX64 Then
	...
	While Not $rs.EOF
		...
		If $zeile > _GUICtrlListView_GetItemCount($computer) - 1 Then GUICtrlCreateListViewItem("", $computer)
		_GUICtrlListView_AddSubItem($computer, $zeile, $x, 4)
		_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("ip_address").Value, 3), 5)
		_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("mac_address").Value, 3), 6)
		If Not Ping($x, 50) Then
			_GUICtrlListView_AddSubItem($computer, $zeile, "off", 7)
		Else
			_GUICtrlListView_AddSubItem($computer, $zeile, "on", 7)
		EndIf
		$zeile += 1
		If $zeile > 9 Then ExitLoop
		$rs.MoveNext
	...
	While Not $rs.EOF
		...
		If $bool = 0 Then
			...
			$x = StringStripWS($rs.Fields("name").Value, 3)
			If $zeile > _GUICtrlListView_GetItemCount($computer) - 1 Then GUICtrlCreateListViewItem("", $computer)
			_GUICtrlListView_AddSubItem($computer, $zeile, $tmp, 4)
			_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("ip_address").Value, 3), 5)
			_GUICtrlListView_AddSubItem($computer, $zeile, StringStripWS($rs.Fields("mac_address").Value, 3), 6)
			If Not Ping($x, 50) Then
				_GUICtrlListView_AddSubItem($computer, $zeile, "off", 7)
...
Func suche_postkorb($suchbegriff)
	Return
