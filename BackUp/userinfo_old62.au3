...
Func searchDepartmentUsers()
	...
	$userList = GUICtrlCreateListView("Office|Username|UserID|Telephone", 5, 10, 540, 400, $LVS_SORTASCENDING +$LVS_SINGLESEL)
	...
	$btnNeighboursSwitch = GUICtrlCreateButton("Switch to this neighbour", 5, 440, 540, 20)
	...
	Do
		...
		If Not StringInStr($y, "ou=Recipients") Then
			...
			GUICtrlCreateListViewItem($o_temp_Usr.physicalDeliveryOfficeName & "|" & $o_temp_Usr.sn & "," & $o_temp_Usr.givenName & "|" & $o_temp_Usr.samAccountName & "|" & $o_temp_Usr.telephoneNumber , $userList)
		EndIf
		$objRecordSet.MoveNext
	Until $objRecordSet.EOF
...
;~ 	Next
	...
	While 1
		$msg = GUIGetMsg()
		If ($msg = $btnNeighboursSwitch) Or ($msg = $btnNeighbours_compare) Then
			$x = GUICtrlRead($userList)
			If $x <> "" Then ; nur wenn auch ein User gewählt wurde...
				$y = GUICtrlRead($userList) ; die Control-ID des markierten Listview-Items holen
				$y = GUICtrlRead($y) ; das ListviewItem auslesen
				$tmp = StringSplit($y, "|") ; aufteilen in einzelne Felder
				If $msg = $btnNeighboursSwitch Then
