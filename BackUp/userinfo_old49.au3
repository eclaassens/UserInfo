...
Func _ADGetGroupMembers($group)
	...
	SplashTextOn("", "Please wait while minions are rumbling through the AD and finding the members", "-1", "-1", "-1", "-1", 33, "", "", "")
	...
	For $strMember = 0 To UBound($arrayMembers) - 1
		$result1 = $result1 & "<td>" & $arrayMembers[$counter][0] & "<br>(" & $arrayMembers[$counter][1] & ")</td>"
		$counter = $counter + 1
	...
	For $strMember = 0 To UBound($arrayMembers) - 1
		...
		$result2 = $result2 & "<td>" & $arrayMembers[$counter][1] & "<br>(" & $arrayMembers[$counter][0] & ")</td>"
		$counter = $counter + 1
	...
	FileWriteLine($data, "<html><head><style>")
	...
	FileWriteLine($data, "</style></head><body> <table class=table align=center><tbody>")

	FileWriteLine($data, "<tr><th>" & "Groupname" & ":</th><td colspan=10>" & $group & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Created" & ":</th><td colspan=10>" & $created & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Last changed" & ":</th><td colspan=10>" & $changed & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "mail" & ":</th><td colspan=10>" & $objGroup.mail & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Group info" & ":</th><td colspan=10>" & $objGroup.info & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Description" & ":</th><td colspan=10>" & $objGroup.description & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Scope of the group" & ":</th><td colspan=10>" & $x & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Group class" & ":</th><td colspan=10>" & $z & "</td></tr>")
	FileWriteLine($data, "<tr><th>" & "Member count" & ":</th><td colspan=10>" & UBound($arrayMembers) - 1 & "</td></tr>")
	;todo: remove groupmembers by name...who cares
		;FileWriteLine($data, "<tr><th bgcolor=ff6666>" & "Groupmembers by Name" & ":</th>" & $result1 & "</tr>")
	FileWriteLine($data, "<tr><th>Groupmembers:</th>" & $result2 & "</tr>")
	FileWriteLine($data, "</tbody></table></body></html>")
	FileClose($data)
...
Func compareGroup($compareUser = "")
	...
	FileWriteLine($data, ".table tfoot td{ padding:2px 0px; color:#666; text-align:center; font-size:10px; }")
	FileWriteLine($data, "</style>")
	FileWriteLine($data, "<html> <body> <table class=table align=center>")
	; write header
		...
		Switch $nMsg
			...
			Case $btnGroupMembers
				...
				If $x <> "" Then ; nur wenn auch ein User gewählt wurde...
					...
					$tmp = StringSplit($y, "|") ; strip everything but UserID
					_GUICtrlStatusBar_SetText($sbMain, "Getting group members for " & $tmp & "...this can take a while.")
