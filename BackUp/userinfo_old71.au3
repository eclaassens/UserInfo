...
;~ 	Next
	...
	While 1
		...
		If ($msg = $btnColleagueSwitch) Or ($msg = $btnColleagueCompare) Then
			$x = GUICtrlRead($userList)
			ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $userList = ' & $x & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
			If $x <> "" Then ; test for empty selection
				$y = GUICtrlRead($userList)
				ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $userList = ' & $y & @CRLF & '>Error code: ' & @error & @CRLF) ;### Debug Console
				$y = GUICtrlRead($y)
